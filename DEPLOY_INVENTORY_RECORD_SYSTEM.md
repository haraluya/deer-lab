...
#### 問題確認 (2025-08-31 09:22)
- [x] **問題現象**：所有盤點記錄使用舊格式（單項目記錄），而非新的 `details` 陣列格式
- [x] **影響範圍**：香精管理、物料管理的盤點功能
- [x] **記錄特徵**：
  - `changeReason: 'inventory_check'`
  - `remarks: '透過盤點模式調整庫存'`
  - `details` 陣列為空，但有舊格式欄位：`itemId`, `itemType`, `itemCode`, `itemName`, `quantityChange`, `quantityAfter`

#### 診斷結果
- [x] **前端函數檢查**：`createInventoryRecordByReason` 格式正確，使用 `details` 陣列
- [x] **後端函數檢查**：`performStocktake` 程式碼正確，應建立新格式記錄
- [x] **實際記錄檢查**：仍為舊格式，表示有其他程式碼路徑在建立記錄
- [x] **關鍵線索**：記錄備註「透過盤點模式調整庫存」在現有程式碼中找不到

#### 待解決
- [ ] **找出建立舊格式記錄的程式碼路徑**
- [ ] **確認是否有其他版本的 Firebase Functions 在運行**
- [ ] **檢查前端是否有其他函數直接建立記錄**
