...
### 0. 🎯 統一 ID 系統 (2025-09-13 完成) - AI 必讀 ⚠️
**🚨 絕對重要：employeeId = Firebase Auth UID = Firestore 文檔ID = personnelId**

#### 統一標準 (100% 實施完成)
- ✅ **所有用戶 ID 完全統一**：一個用戶只有一個 ID，在所有地方都相同
- ✅ **範例**：用戶「哈雷雷」的所有 ID 都是 `"052"`
- ✅ **映射關係**：
  ```typescript
  employeeId: "052"       // 員工編號
  Firebase Auth UID: "052" // 登入認證 ID
  Firestore 文檔ID: "052" // 資料庫文檔 ID
  personnelId: "052"      // 工時記錄使用的 ID
  ```

#### AI 開發指南
1. **🚫 絕對禁止**：建立任何 ID 映射或轉換邏輯
2. **✅ 正確做法**：直接使用任一 ID，它們都相同
   ```typescript
   // ✅ 正確：直接使用
   const userId = appUser.employeeId; // 或 appUser.uid，都相同
...
3. **📍 關鍵檔案位置**：
   - 前端工具：`src/utils/uid-manager.ts`
   - 後端中間件：`functions/src/middleware/uid-validator.ts`
   - 驗證腳本：`test-unified-id-system.js`

4. **🔧 API 參數處理標準**：
   ```typescript
   // 統一的 fallback 邏輯
   const finalId = employeeId || userId || firebaseUid;
   ```

5. **⚠️ 系統完整性**：已通過 100% 測試，A+ 評分
   - 所有 10 筆用戶記錄完全一致
   - 工時記錄系統完美映射
   - Firebase Auth 與 Firestore 完全同步
