...
15. **採購系統效能與顯示優化** (2025-09-05):
   - **加入購物車速度優化**: 實作樂觀更新機制，立即更新本地狀態並顯示成功提示
   - **背景同步**: 加入購物車後在背景同步到 Firebase，不阻塞用戶操作
   - **採購單金額計算修復**: 修復採購單清單中總金額顯示問題
   - **金額計算邏輯**: 自動計算項目總額（costPerUnit × quantity）和其他費用總額
