...
function InventoryPageContent() {
  ...
  const handleInventoryCheck = async () => {
    ...
    try {
      ...
      // 使用 Firebase Functions 進行庫存盤點
      const functions = getFunctions()
      const performStocktake = httpsCallable(functions, 'performStocktake')
      
      const stocktakeData = {
        items: [{
          itemRefPath: `${itemType === "material" ? "materials" : "fragrances"}/${itemId}`,
          currentStock: oldStock,
          newStock: newStock
        }]
      }
      
      console.log('呼叫 performStocktake 函數，資料:', stocktakeData)
      
      const result = await performStocktake(stocktakeData)
      console.log('performStocktake 結果:', result)
      ...
      toast.error(`庫存盤點失敗: ${errorMessage}`)
    }
  }
