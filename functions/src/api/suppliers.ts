...
export const createSupplier = onCall(async (request) => {
  ...
  try {
    const newSupplier = { name, contactPerson: contactPerson || "", phone: phone || "", address: address || "", createdAt: FieldValue.serverTimestamp(), updatedAt: FieldValue.serverTimestamp(), };
    const docRef = await db.collection("suppliers").add(newSupplier);
    logger.info(`管理員 ${contextAuth?.uid} 成功建立新供應商: ${docRef.id}`);
    return { status: "success", message: `供應商 ${name} 已成功建立。`, supplierId: docRef.id, };
  } catch (error) { logger.error("建立供應商時發生錯誤:", error); throw new HttpsError("internal", "建立供應商時發生未知錯誤。"); }
});
...
export const updateSupplier = onCall(async (request) => {
  ...
  try {
    ...
    const updateData = { name, contactPerson: contactPerson || "", phone: phone || "", address: address || "", updatedAt: FieldValue.serverTimestamp(), };
    await supplierRef.update(updateData);
    logger.info(`管理員 ${contextAuth?.uid} 成功更新供應商資料: ${supplierId}`);
    return { status: "success", message: `供應商 ${name} 的資料已成功更新。`, };
  } catch (error) { logger.error(`更新供應商 ${supplierId} 時發生錯誤:`, error); throw new HttpsError("internal", "更新供應商資料時發生未知錯誤。"); }
});
...
export const deleteSupplier = onCall(async (request) => {
  ...
  try {
    ...
    return { status: "success", message: "供應商已成功刪除。", };
  } catch (error) { logger.error(`刪除供應商 ${supplierId} 時發生錯誤:`, error); throw new HttpsError("internal", "刪除供應商時發生未知錯誤。"); }
});
