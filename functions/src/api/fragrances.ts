...
export const fixAllFragranceRatios = onCall(async (request) => {
  ...
  try {
    ...
    fragrancesQuery.docs.forEach(doc => {
      ...
      if (fragrancePercentage > 0) {
        ...
        if (Math.abs(currentPgRatio - pgRatio) > 0.1 || Math.abs(currentVgRatio - vgRatio) > 0.1) {
          ...
          fixedCount++;
          logger.info(`修正香精 ${data.name} (${data.code}) 比例: 香精=${fragrancePercentage}%, PG=${currentPgRatio}->${pgRatio}%, VG=${currentVgRatio}->${vgRatio}%`);
        }
      }
    });
    ...
    if (fixedCount > 0) {
      await batch.commit();
      logger.info(`批量修正完成，共修正 ${fixedCount} 個香精的比例`);
    } else {
      logger.info('所有香精比例都正確，無需修正');
    }
    ...
    logger.error('批量修正香精比例失敗:', error);
    throw new HttpsError('internal', '批量修正香精比例時發生錯誤');
  }
});

// 診斷香精比例問題
export const diagnoseFragranceRatios = onCall(async (request) => {
  const { auth: contextAuth } = request;
  // await ensureIsAdmin(contextAuth?.uid);
  
  try {
    logger.info('開始診斷香精比例...');
    
    // 獲取所有香精
    const fragrancesQuery = await db.collection('fragrances').get();
    const problematicFragrances: any[] = [];
    const correctFragrances: any[] = [];
    ...
    logger.info(`診斷完成：總共 ${fragrancesQuery.docs.length} 個香精，${problematicFragrances.length} 個比例錯誤，${correctFragrances.length} 個比例正確`);
    ...
    logger.error('診斷香精比例失敗:', error);
    throw new HttpsError('internal', '診斷香精比例時發生錯誤');
  }
});
